...
const express = require('express');
const router = express.Router();
const { criarInstancia, gerarQRCode } = require('../services/evolutionService');
const { EmpresaRepository } = require('../database');
...
router.post('/whatsapp/conectar', async (req, res) => {
  try {
    ...
    const instanceName = `empresa_${empresa_id}`;

    // Criar instância (se já existir, ignora)
    try {
      await criarInstancia(instanceName);
    } catch (e) {
      console.log('ℹ️ Instância pode já existir');
    }
    ...
    // Salvar no banco
    await EmpresaRepository.atualizarWhatsApp(empresa_id, {
    ...
    console.error('Erro conectar WhatsApp:', error.message);
...
/**
 * POST /api/admin/whatsapp/conectar
 * Cria instância Evolution e retorna QR Code
 */
router.post('/whatsapp/conectar', async (req, res) => {
  try {
    const empresa_id =
    ...
    // Nome único da instância por empresa
    const instanceName = `rebeca_empresa_${empresa_id}`;

    console.log('📱 Iniciando WhatsApp da empresa:', instanceName);

    // Criar instância (se já existir, Evolution ignora)
    await criarInstancia(instanceName);
